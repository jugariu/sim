...
import com.sim.image.fusion.ImageFusion;
import com.sim.ui.components.ScrollableLogArea;

public class SelectMaximumImageFusion implements ImageFusion {
	...
	public SelectMaximumImageFusion(String workingDirPath, ScrollableLogArea log) {
		this.workingDirPath = workingDirPath;
		this.log = log;
		
		log.setLoggedClass(SelectMaximumImageFusion.class.getName());
	}
	...
	public String processImages(String firstImagePath, String secondImagePath) {
		...
		cronometer.start();
		log.info("Select Maximum Image Fusion processing started.");
	...
	private BufferedImage getSelectedMaximumImageFusionDiff(BufferedImage first, BufferedImage second, int xDiff, int yDiff) {
		for (int y = 0; y < second.getHeight(); y++) {
			for (int x = 0; x < second.getWidth(); x++) {
				try{
					...
					int redFusionedValue = getMax(redList);
					int greenFusionedValue = getMax(greenList);
					int blueFusionedValue = getMax(blueList);
