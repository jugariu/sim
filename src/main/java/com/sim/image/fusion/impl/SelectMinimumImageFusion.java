...
public class SelectMinimumImageFusion {
	...
	public String processImages(String firstImagePath, String secondImagePath) {
		...
		cronometer.stop();
		log.appendInfo("Select Minimum Image Fusion processing finished in " + cronometer.getTime() + "ms.");
	...
	private BufferedImage getSelectedMinimumImageFusionDiff(BufferedImage first, BufferedImage second, int xDiff, int yDiff) {
		for (int y = 0; y < second.getHeight(); y++) {
			for (int x = 0; x < second.getWidth(); x++) {
				try{
					...
					int redFusionedValue = getMin(redList);
					int greenFusionedValue = getMin(greenList);
					int blueFusionedValue = getMin(blueList);
